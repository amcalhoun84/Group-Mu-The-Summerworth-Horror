...
void createObject(Room room, vector<Room> roomStorage)
{
    ...
    while (getline(roomFile, roomData, ';'))
	{
		switch(counter)
		{
            ...
            //This is the roomId
            case 2:
                room.setRoomID(atoi(roomData.c_str(roomData)));  //In C++11 this will convert a string to an integer. If C++11 does not work on flip use: atoi(roomData.c_str())
                break;
            case 3:
                room.setNorth(atoi(roomData.c_str(roomData)));
                break;
            case 4:
                room.setSouth(atoi(roomData.c_str(roomData)));
                break;
            case 5:
                room.setEast(atoi(roomData.c_str(roomData)));
                break;
            case 6:
                room.setWest(atoi(roomData.c_str(roomData)));
                break;
            case 7:
                if(roomData == "0")
                {
